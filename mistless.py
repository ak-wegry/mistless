...
# Ver1.01  2022/12/04 java対応の追加
#                     chardetの文字コード判定誤り(可能性)への対処
#                     対応する括弧を探す処理の不具合修正
#                     C/Javaでのタグ作成処理の不具合修正
# Ver1.02  2022/12/29 Windows/Linuxコードの共通化
#                     罫線素片の表示ズレ補正
# Ver1.03  2023/10/29 class内でのグローバル変数参照の解消
# Ver1.04  2023/11/26 ()の無いクラス/関数定義でタグ作成する修正
#                     0byteファイルで異常終了する不具合修正
#                     リストファイルによる読込み機能の追加
#                     ファイル読込み時のタグ検索文字列を行番号へ変更
#                     標準入力からの読込み機能追加
# Ver1.05  2023/12/03 ESCを含む行の表示不具合を修正
#                     syntax強調パターン切替え機能追加
#                     引数で指定した全ファイル分のタグ作成する機能追加
...
usage = """
プログラム・トレース・ツール  [Ver1.05  2023/12/3]
			...
			if opt_id == "":
				...
				if opt_id == "-m":
					...
					if Path(arg).exists() is True:
						...
						self.err_msg = file_msg % ("タグ", arg)
						break
					opt_id = ""
				elif opt_id == "-l":
					# リストファイルによる読込みファイル名の設定
					if Path(arg).exists() is True:
...
class DispCtrl():
	...
	Attributes:
		...
		srch_str   : 検索の文字列
		srch_dir   : 検索の方向
		history    : HistoryCtrlクラス参照用
		keyctrl    : KeyCtrlクラス参照用
		taginfo    : TagInfoクラス参照用
	"""
	...
	def clear_eol(self):
		print("\x1b[0K", end="") # カーソル位置から右側消去

	def clear_eob(self):
		print("\x1b[0J", end="") # カーソル位置から画面右下まで消去
...
class FileCtrl():
	...
	def unicode_len(self, text):
		...
		width = 0
		for char in text:
			if re.fullmatch(r"[┌┬┐├┼┤└┴┘│]", char) is not None:
				width += 1
	...
	def judge_border(self, cur_pos, add_len, border_pos):
		...
		Returns:
			touch (bool): True:境界位置に接地(跨ぐ場合含む)
	...
	def next_line(self, move_cnt):
		...
		#Todo: index位置でなく、カーソル位置で比較するように修正
		pre_idx = -1
		iter = re.finditer(r"[a-zA-Z0-9_]+", line)
		for n in iter:
			idx = n.span()[0]
	...
	def select_list(self, tag_list):
		...
		file_width = self.display.term_width - 4 # "No. "の幅を減算
		srch_width = self.display.term_width - 6 # 先頭インデント幅を減算
...
class TagInfo():
	...
	Attributes:
		...
		add_num     : タグ情報.行検索文字に行番号付与の有無(True/False)
	"""
